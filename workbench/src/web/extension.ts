...
async function rewriteShellHistoryArtifacts(fsys: any, entries: ShellHistoryEntry[], generatedAt: Date): Promise<string[]> {
	...
	await fsys.writeFile(SHELL_HISTORY_MD_PATH, shellHistoryLatestMarkdown(entries, generatedAt, {
		note: `This file was regenerated by browser-side history compaction. The JSONL file keeps ${entries.length} retained commands; the grouped summary links each retained command to its evidence file.`,
	}));
	const artifacts = shellHistoryArtifacts(entries);
	await writeShellHistoryCommandArtifacts(fsys, artifacts);
...
async function existingShellHistoryPaths(fsys: any): Promise<string[]> {
	const paths = [SHELL_HISTORY_SUMMARY_MD_PATH, SHELL_HISTORY_COMMANDS_DIR, SHELL_HISTORY_ARCHIVE_DIR, SHELL_HISTORY_SELECTED_MD_PATH, SHELL_HISTORY_MD_PATH, SHELL_HISTORY_JSON_PATH, SHELL_HISTORY_JSONL_PATH];
