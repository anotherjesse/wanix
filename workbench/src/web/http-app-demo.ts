...
// Counter state lives in the #kv device (a namespace binding shared across qjs
// task invocations), not a file under /apps, so the count survives per-request
// task teardown and is inspectable as a Wanix data store.
const COUNTER_STATE_KEY = "http-counter";
const COUNTER_STATE_PATH = `#kv/${COUNTER_STATE_KEY}`;
...
// namespace root, not the task cwd). The key is shared across qjs task
// invocations, so the count persists between HTTP requests.
const key = "#kv/${COUNTER_STATE_KEY}";
let count = 0;
