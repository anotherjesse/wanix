...
> | 6 — `#cpu` exec plane | **COMPLETE** — `wanix-cpu` + `CpuAcceptor`/`CpuDialer` in `wanix-mesh`, the `wanix cpu` dial verb, and `mesh-serve --cpu` serving the acceptor behind the exec gate (refused on the public endpoint; `--peer`-scoped on the local one) |
