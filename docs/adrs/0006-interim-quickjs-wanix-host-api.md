...
`env`, and `dir` task fields. `Wanix.exit` and the fd helpers are installed only
for Wanix task runs. Fd helpers allocate, read, write, and close entries in the
Wanix task fd table; they do not create a separate QuickJS fd namespace. Because
the current QuickJS callback surface copies scalar values, `readFd` and
`writeFd` are text/UTF-8 helpers rather than byte-accurate WASI replacements.
After an exit request, later host-visible output, namespace writes, fd side
effects, and pending jobs are suppressed. Console output continues to flow
through task stdout and stderr fds. This API is an adapter-level bridge, not the
final system call surface.
