...
- `wanix-rust qjs-snapshot [--stdin TEXT | --stdin-file PATH|-]
  [--interrupt-after N] [--memory-limit-bytes N] [--event-loop-ms N]
  [--ready-io-turns N] --snapshot FILE <script.js>` runs JavaScript as a qjs
  Wanix task, writes the captured QuickJS VM image to `FILE`, and reports task
  output through ordinary Wanix stdio fds.
- `wanix-rust qjs-resume [--stdin TEXT | --stdin-file PATH|-]
  [--interrupt-after N] [--memory-limit-bytes N] [--event-loop-ms N]
  [--ready-io-turns N] --snapshot FILE <script.js>` reads the VM image from
...
both commands; ADR 0048 adds explicit interrupt-poll budget reattachment; ADR
0049 adds explicit future-timer and ready-IO budget reattachment. Open dynamic
Wanix task fds and live WASI fds continue to block snapshot creation.
