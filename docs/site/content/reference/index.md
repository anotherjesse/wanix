...
- [**CLI Command Index**](/reference/cli-command-index) — Every wanix subcommand at a glance — qjs, qjs-term/-shell/-snapshot/-resume/-restore, wasm, p9-*, serve, rootfs, qemu, mesh-serve, mount-*, cpu, agent, capsule — each linked to its dedicated reference or recipe page.
...
- [**Contributor Landing — This Is the Rust Port**](/reference/contributor-landing) — Orient: this is the Rust-native workspace — build with cargo, build with cargo, and gate with just check.
