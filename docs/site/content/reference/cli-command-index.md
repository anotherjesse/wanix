...
- **`cpu --node iroh://PEER[?addr=IP:PORT] [--cwd DIR] [--write] [--env KEY=VALUE ...] -- KIND PROGRAM [ARG ...]`** — Plan 9 cpu over the mesh: reverse-export `DIR` (read-only by default; `--write` opts into read-write) and run `KIND PROGRAM` on the data node against it (`help.rs:44-47`). Dial-only today: no serve mode binds the `CpuAcceptor` yet.
