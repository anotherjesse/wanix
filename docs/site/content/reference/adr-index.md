...
usedInFlows: []
honestLimits:
  - The native mesh wire is now recorded in ADR 0004; the rest of the mesh/agent layer (ed25519 identity, capability binds, #agent, and the #kv/#pipe/#plumb/#cas/#cpu device contracts) has no ADRs yet and lives in docs/mesh-blueprint.md and the integration docs.
...
`docs/adrs/0004-rust-9p-protocol-and-server-contract.md` makes the root contract the `FileSystem` trait, not any wire, and chooses encodings per boundary. It splits the wire from the server: `wanix-protocol` owns dependency-free frame splitting, tag extraction, version negotiation, and typed operation codecs (the server-facing 9P2000.L surface plus selected compatibility codecs); `wanix-9p` owns the server state that maps fids to Wanix filesystem objects and translates results into 9P replies and Linux-ish errno errors, leaving listener, socket, stdio, and browser policy to adapters. It also records the **native FileSystem-over-iroh mesh wire** — between two Wanix nodes the mesh uses a hand-rolled `postcard`-framed wire (`wanix-mesh-wire`, the crate; the chosen fallback over `irpc`, recorded with its rationale) with a typed `WireFsError`, one QUIC stream per call/open-file, and per-connection principal identity — while **9P is demoted to the foreign-edge gateway** (Linux/v86/QEMU, external 9P, the cockpit). See [protocol vs. server split](/concepts/protocol-vs-server-split), [the 9P contract](/concepts/the-9p-contract), and [the missing half of 9P](/concepts/missing-half-of-9p).
...
- **The mesh and agent layer is only partly recorded in ADRs.** The native mesh wire is now owned by ADR 0004 (with the op-by-op design in `docs/design/native-mesh-wire.md`), but the ed25519 identity and default-deny capability binds, the [`#agent` device](/devices/agent), and the [`#kv`](/devices/kv)/[`#pipe`](/devices/pipe)/[`#plumb`](/devices/plumb)/[`#cas`](/devices/cas)/[`#cpu`](/devices/cpu) service contracts are not yet recorded as ADRs. Their design lives in `docs/mesh-blueprint.md`, `docs/mesh-the-missing-half-of-9p.md`, and the cockpit↔mesh integration docs under `docs/integration/`. Promote the remaining durable boundaries (mesh identity/trust, agent-as-device, the service-device contracts) into consecutive ADRs when those contracts stabilize.
