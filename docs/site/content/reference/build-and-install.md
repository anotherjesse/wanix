...
pageType: reference
oneLiner: "The one canonical page for building wanix from source: toolchain, the build command, the alias, and the optional wasm32-wasip1 target."
...
usedInFlows: []
honestLimits:
  - "There is no packaged Rust-native release yet. Build the CLI from source."
canonicalCaveatFor: []
---

# Build & Install the Wanix CLI

Every flow and recipe on this site runs the native CLI, `wanix`. This page is the one place its build steps live; the other pages link here instead of repeating them.

Historical note: the original Wanix was a Go/browser implementation hosted at
https://github.com/tractordev/wanix/. On this site, `wanix` means the
Rust-native CLI/core built from this workspace.
...
The docs write `wanix ...` everywhere. Make that work in your shell:

```sh
alias wanix='./target/debug/wanix'
```

(Or use `WANIX=./target/debug/wanix` and `$WANIX ...` — same thing.) The alias assumes you stay in the workspace root; use the absolute path if you wander.
...
`wanix --help` lists every subcommand; the [CLI command index](/reference/cli-command-index) maps each one to its docs.
...
- There is no packaged release of the Rust-native CLI yet; build from source as above.
