...
ADR 0004 makes the boundary rule explicit: stdio, TCP, WebSocket, and `serve` are "adapters over the same server contract. They must preserve binary frame boundaries and keep diagnostic/status output out of binary response streams" (`docs/adrs/0004-rust-9p-protocol-and-server-contract.md:41-43`). That is why `wanix-rust p9-stdio`, `p9-listen`, and `p9-ws` differ only in their byte plumbing, never in what `Tread` means.
