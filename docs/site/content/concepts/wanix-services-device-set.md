...
- `--wanix-services` binds the `#task`/`#agent` exec devices — remote code execution. The mesh path keeps exec local-trust only, so `mesh-serve --wanix-services` is refused on the public endpoint even with `--peer`/`--grant`; the one allowed path is a direct-address `--addr IP:PORT` socket whose ticket is exchanged out of band (`crates/wanix-cli/src/mesh/serve.rs:120-139`). This buys cheap, scalable isolation, not safety for arbitrary untrusted code: there are no hard CPU or memory limits yet.
