...
This is not purely speculative anymore. The cpu branch ships a concrete first slice: the [`#plumb`](/devices/plumb) device and the [qjs-shell](/concepts/qjs-shell) mutation frame implement a traceable plumber boundary, so shell-driven mutations and external effects flow through one observable wire and namespace changes can be correlated with the commands that caused them (`docs/traceable-dynamic-namespaces.md:1-9`).
