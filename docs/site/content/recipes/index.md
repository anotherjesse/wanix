...
- [**Recipe 06 — Compose a Volume and Two Tools in One Shell Across the Mesh**](/recipes/06-compose-volume-and-tools) — Serve a demo-notes volume and the upper/sha256 tools as three mesh resources, mount all three into one wanix sh namespace, pipe a remote file through a remote tool back into the remote volume, then drive the raw job protocol with no sugar.
