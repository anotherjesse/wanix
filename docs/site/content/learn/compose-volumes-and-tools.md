...
wanix tool serve --tool upper --tool sha256 --addr 127.0.0.1:0
```
...
The `tool` builtin (`crates/wanix-sh/src/tool.rs`) is sugar, not a protocol. Underneath, a call is a *job directory* you can drive by hand — ADR 0009's calling convention (`docs/adrs/0009-job-protocol.md`). Run the interactive REPL (`wanix sh` with no `-c`) against the upper mount and do what the builtin does, file by file:
...
```sh
wanix sh -c 'cat < /n/upper/new' --mount-mesh 'iroh://a2887734...=/n/upper'
# j31770e20a12797a3
wanix sh -c 'cat /n/upper/jobs/j31770e20a12797a3/status' --mount-mesh 'iroh://a2887734...=/n/upper'
