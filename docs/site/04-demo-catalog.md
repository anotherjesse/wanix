...
- **Shows:** `p9-listen --peer --grant` installs a default-deny GrantTable; an authorized peer attaches a `SubtreeFs`-scoped read-write root; an unmatched attach gets EACCES (errno 13).
...
- **Media:** RECORDED asciinema/screencast (needs two endpoints; partly proven only at serve_stream-level loopback per research, so frame as a recorded/annotated transcript, not a "paste this" live cast). **Source:** `docs/mesh-the-missing-half-of-9p.md:712-851`; `crates/wanix-id/src/grant.rs`; `wanix p9-listen`.
