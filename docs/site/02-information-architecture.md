...
1. **Developer-track landing** — Orient: this is the Rust-native workspace — build with cargo; use just check.  _(`reference/contributor-landing`)_
...
| `Everything is a file` | prerequisite-of | `Porting a normal binary (what a task sees)` |
| `Command-style wasm linker (poll_oneoff = NOSYS)` | prerequisite-of | `Porting a normal binary (what a task sees)` |
| `Porting a normal binary (what a task sees)` | related-to | `Per-process namespaces` |
| `Porting a normal binary (what a task sees)` | related-to | `Host files are not ambient authority` |
| `Porting a normal binary (what a task sees)` | related-to | `Wanix-backed WASI (not host WASI)` |
