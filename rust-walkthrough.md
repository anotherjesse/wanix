...
> plus a synchronous `qjs SCRIPT [ARGS...] [< STDIN]` launcher that allocates a
> child `qjs` task through `#task/new/qjs`, wires stdout/stderr to the
> terminal, optionally binds fd `0` from a namespace file, and waits on the
> child's `exit` file.
