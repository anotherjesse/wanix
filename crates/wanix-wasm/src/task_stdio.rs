//! Builds a [`WasiConfig`] from a Wanix [`Task`].
//!
//! This mirrors the qjs task-stdio wiring: the task namespace backs WASI, the
//! task cwd is the root preopen source, argv/env come from the shared
//! [`wanix_task`] command extraction, and task fds 0/1/2 are wired as guest
//! stdio. Dynamically opened guest fds are mirrored into the task fd table
//! through a [`WasiFdObserver`] so they are observable at the task level and
//! released on guest close.
