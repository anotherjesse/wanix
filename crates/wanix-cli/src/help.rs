...
pub(super) const USAGE: &str = concat!(
    ...
    "       wanix-rust tool serve --tool NAME [--tool NAME ...] [--addr IP:PORT] [--insecure-open]\n",
    "         (built-in ToolFS devices over the native mesh — one endpoint + ticket per tool, ",
    "use --addr port 0 for multiple; tools: model (deterministic fake), sha256, upper; ",
    "every connection sees only its own jobs, keyed by the verified peer id)\n",
    "       wanix-rust app serve --app DIR --state DIR [--name NAME] [--addr IP:PORT] [--insecure-open]\n",
    ...
    "mounted at /state; attribution and presence come from the verified peer id, never the payload)\n",
