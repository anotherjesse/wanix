...
impl QuickJsWasiHost for MetadataWasiHost {
    ...
    fn path_remove_directory(&mut self, dirfd: u32, path: &[u8]) -> WasiHostResult<()> {
        self.record(format!("rmdir:{dirfd}:{}", String::from_utf8_lossy(path)));
        if path == b"nonempty" {
...
#[test]
fn live_wasi_host_supplies_fd_filestat_set_times() -> Result<()> {
    let host = MetadataWasiHost::default();
    let calls = Arc::clone(&host.calls);
    let mut harness =
        VirtualFsHarness::new_with_wasi_host(QuickJsHostConfig::new(), Some(Box::new(host)))?;
    ...
    assert_eq!(
        harness
            ...
            .call(&mut harness.store, (44, 1_000, 2_000, 5))?,
        ERRNO_SUCCESS
    );
...
#[test]
fn live_wasi_host_supplies_path_create_directory() -> Result<()> {
    let host = MetadataWasiHost::default();
    let calls = Arc::clone(&host.calls);
    let mut harness =
        VirtualFsHarness::new_with_wasi_host(QuickJsHostConfig::new(), Some(Box::new(host)))?;
    harness.write_bytes(PATH_PTR, b"newdir")?;

    assert_eq!(
        harness
            .path_create_directory
            .call(&mut harness.store, (9, 128, 6))?,
        ERRNO_SUCCESS
    );

    assert_eq!(
        calls.lock().expect("test call lock").as_slice(),
        &["mkdir:9:newdir".to_owned()]
    );
    Ok(())
}
...
fn live_wasi_host_supplies_path_remove_directory() -> Result<()> {
    ...
    let mut harness =
        VirtualFsHarness::new_with_wasi_host(QuickJsHostConfig::new(), Some(Box::new(host)))?;
    harness.write_bytes(PATH_PTR, b"empty")?;
    harness.write_bytes(PATH_PTR + 16, b"nonempty")?;

    assert_eq!(
        harness
            .path_remove_directory
            .call(&mut harness.store, (9, 128, 5))?,
        ERRNO_SUCCESS
    );
    assert_eq!(
        harness
            .path_remove_directory
            .call(&mut harness.store, (9, 144, 8))?,
        ERRNO_NOTEMPTY
    );

    assert_eq!(
        calls.lock().expect("test call lock").as_slice(),
        &["rmdir:9:empty".to_owned(), "rmdir:9:nonempty".to_owned()]
    );
    Ok(())
}
...
#[test]
fn fd_filestat_set_times_is_unsupported_for_virtual_fs_fallback() -> Result<()> {
    let mut harness = VirtualFsHarness::new(virtual_fs_config()?)?;
    assert_eq!(
        harness
            .fd_filestat_set_times
            .call(&mut harness.store, (PREOPEN_ROOT_FD, 1_000, 2_000, 5))?,
        ERRNO_NOSYS
    );
    ...
    assert_eq!(
        harness
            .fd_filestat_set_times
            .call(&mut harness.store, (fd, 1_000, 2_000, 5))?,
        ERRNO_NOSYS
    );
    assert_eq!(
        harness
            .fd_filestat_set_times
            .call(&mut harness.store, (99, 1_000, 2_000, 5))?,
        ERRNO_BADF
    );
    assert_eq!(
        harness
